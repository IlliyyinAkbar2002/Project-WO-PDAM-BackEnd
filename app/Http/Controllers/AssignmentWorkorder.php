...
class AssignmentWorkorder extends Controller
{
    ...
    public function show($id)
    {
        ...
        $assignment = $workorder->workorderAssignment()
            ->with(['spv', 'members.user', 'location'])
            ->first();

        if (! $assignment) {
            return response()->json(['message' => 'Workorder ini belum memiliki assignment.'], 404);
        }

        return response()->json(['message' => 'Data assignment ditemukan.', 'data' => $assignment]);
    }

    public function assignStaff(Request $request, $id)
    {
        $workorder = Workorder::with('jenisWorkorder', 'workorderAssignment.members')->findOrFail($id);
        ...
        $validated = $request->validate([
            ...
            'petugas.*.pegawai_id' => 'required|exists:m_pegawai,id',
        ...
        try {
            $this->assignmentService->assignStaff($workorder, $validated, $request->user());

            $fresh = Workorder::with([
                'assignmentMembers.user',
                'assignedTo',
                'jenisWorkorder',
                'meter',
                'jaringan',
            ...
            return response()->json(['message' => 'Assign staff berhasil', 'data' => $fresh], 200);
    ...
    {
        $fieldMap = [
            'meter'         => ['nomor_meter', 'kondisi_meter_awal', 'kondisi_meter_akhir', 'hasil_kalibrasi'],
            'jaringan'      => ['jenis_pipa', 'diameter_pipa', 'panjang_pipa', 'tingkat_kerusakan', 'tindakan_perbaikan', 'hasil_inspeksi'],
            'infrastruktur' => ['nama_aset', 'jenis_aset', 'kapasitas', 'kondisi_awal', 'kondisi_akhir', 'jadwal_pemeliharaan', 'tindakan'],
        ];
